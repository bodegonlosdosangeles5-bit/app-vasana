...
- [x] Funcionalidad Modal "Compartir Credenciales por QR" nativo sobre `UserAdminPanel`.
- [x] Roles funcionales (soporte nativo para Admin, User, Consulta).
- [x] Autenticación Serverless Segura: Migración del flujo de inicio de sesión a funciones Vercel locales/remotas (`/api/login`) usando `service_role` para un acceso robusto.
...
- **Seguridad y Autenticación (Backend Serverless):** Se implementó un cliente administrador estricto en `lib/supabaseAdmin.ts`. El componente `AuthProvider.tsx` ahora verifica usuarios enviando la solicitud a `/api/login`, una función Serverless que corre bajo Vercel CLI (`vercel dev`), garantizando que la lectura a la base de datos se haga con la clave `service_role` (saltando los bloqueos públicos de RLS).
